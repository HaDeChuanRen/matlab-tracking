...
addpath(genpath('F:\2025年02月-056A对039B\ap\MultiBeam\'));

% load('Data_2x6B_3_BF.mat');
% Tra_mat = cbf3;
Tra_mat = [];
load('MultiBeam_20250226020525.mat');
Tra_mat = [Tra_mat;gather(MB)'];
% load('MultiBeam_20250226020550.mat');
% Tra_mat = [Tra_mat;gather(MB)'];
% load('MultiBeam_20250226020615.mat');
% load('MultiBeam_20250226030408.mat');
% Tra_mat = [Tra_mat;gather(MB)'];
Tra_mat = flipud(Tra_mat);
...
Tra_hyp = TraHypothesis(Tar_set, Clu_set, No_tar, Pro_ini, time_cnt, ...
    Tar_his,Sigma_vec, P_G, L_ite, T_ver, Max_hyp, 12);
Hypset_last = Tra_hyp;
...
% save('XuMenghuai_7_FZBTR4.mat', 'Tra_hyp', 'cbf_n');
