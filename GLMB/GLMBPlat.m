classdef GLMBPlat
    ...
    methods
        function obj = GLMBPlat()
            % GLMBPLAT 构造此类的实例
            %   此处显示详细说明

            % model parameters
            % basic parameters
            obj.model.x_dim= 5;   %dimension of state vector
            obj.model.z_dim= 2;   %dimension of observation vector
            obj.model.v_dim= 3;   %dimension of process noise
            obj.model.w_dim= 2;   %dimension of observation noise
            ...
            obj.model.T= 1;                         %sampling period
            obj.model.sigma_vel= 5;
            obj.model.sigma_turn= (pi/180);   %std. of turn rate variation (rad/s)
            obj.model.bt= obj.model.sigma_vel*[ (obj.model.T^2)/2; obj.model.T ];
            obj.model.B2= [ obj.model.bt zeros(2,2); zeros(2,1) obj.model.bt zeros(2,1); zeros(1,2) obj.model.T*obj.model.sigma_turn ];
            obj.model.B= eye(obj.model.v_dim);
            obj.model.Q= obj.model.B*obj.model.B';
            
            % survival/death parameters
            obj.model.P_S= .99;
            obj.model.Q_S= 1-obj.model.P_S;
            
            % birth parameters (LMB birth model, single component only)
            obj.model.T_birth= 4;         %no. of LMB birth terms
            obj.model.L_birth= zeros(obj.model.T_birth,1);                                          %no of Gaussians in each LMB birth term
            obj.model.r_birth= zeros(obj.model.T_birth,1);                                          %prob of birth for each LMB birth term
            obj.model.w_birth= cell(obj.model.T_birth,1);                                           %weights of GM for each LMB birth term
            obj.model.m_birth= cell(obj.model.T_birth,1);                                           %means of GM for each LMB birth term
            obj.model.B_birth= cell(obj.model.T_birth,1);                                           %std of GM for each LMB birth term
            obj.model.P_birth= cell(obj.model.T_birth,1);                                           %cov of GM for each LMB birth term
            
            obj.model.L_birth(1)=1;                                                             %no of Gaussians in birth term 1
            obj.model.r_birth(1)=0.02;                                                          %prob of birth
            obj.model.w_birth{1}(1,1)= 1;                                                       %weight of Gaussians - must be column_vector
            obj.model.m_birth{1}(:,1)= [ -1500; 0; 250; 0; 0 ];                                 %mean of Gaussians
            obj.model.B_birth{1}(:,:,1)= diag([ 50; 50; 50; 50; 6*(pi/180) ]);                  %std of Gaussians
            obj.model.P_birth{1}(:,:,1)= obj.model.B_birth{1}(:,:,1)*obj.model.B_birth{1}(:,:,1)';      %cov of Gaussians
            
            obj.model.L_birth(2)=1;                                                             %no of Gaussians in birth term 2
            obj.model.r_birth(2)=0.02;                                                          %prob of birth
            obj.model.w_birth{2}(1,1)= 1;                                                       %weight of Gaussians - must be column_vector
            obj.model.m_birth{2}(:,1)= [ -250; 0; 1000; 0; 0 ];                                 %mean of Gaussians
            obj.model.B_birth{2}(:,:,1)= diag([ 50; 50; 50; 50; 6*(pi/180) ]);                  %std of Gaussians
            obj.model.P_birth{2}(:,:,1)= obj.model.B_birth{1}(:,:,1)*obj.model.B_birth{1}(:,:,1)';      %cov of Gaussians
            
            obj.model.L_birth(3)=1;                                                             %no of Gaussians in birth term 3
            obj.model.r_birth(3)=0.03;                                                          %prob of birth
            obj.model.w_birth{3}(1,1)= 1;                                                       %weight of Gaussians - must be column_vector
            obj.model.m_birth{3}(:,1)= [ 250; 0; 750; 0; 0 ];                                   %mean of Gaussians
            obj.model.B_birth{3}(:,:,1)= diag([ 50; 50; 50; 50; 6*(pi/180) ]);                  %std of Gaussians
            obj.model.P_birth{3}(:,:,1)= obj.model.B_birth{1}(:,:,1)*obj.model.B_birth{1}(:,:,1)';      %cov of Gaussians
            
            obj.model.L_birth(4)=1;                                                             %no of Gaussians in birth term 4
            obj.model.r_birth(4)=0.03;                                                          %prob of birth
            obj.model.w_birth{4}(1,1)= 1;                                                       %weight of Gaussians - must be column_vector
            obj.model.m_birth{4}(:,1)= [ 1000; 0; 1500; 0; 0 ];                                 %mean of Gaussians
            obj.model.B_birth{4}(:,:,1)= diag([ 50; 50; 50; 50; 6*(pi/180) ]);                  %std of Gaussians
            obj.model.P_birth{4}(:,:,1)= obj.model.B_birth{1}(:,:,1)*obj.model.B_birth{1}(:,:,1)';      %cov of Gaussians
            ...
            obj.model.D= diag([ 2*(pi/180); 10 ]);      %std for angle and range noise
            obj.model.R= obj.model.D*obj.model.D';              %covariance for observation noise
            
            % detection parameters
            obj.model.P_D= .98;   %probability of detection in measurements
            obj.model.Q_D= 1-obj.model.P_D; %probability of missed detection in measurements
            
            % clutter parameters
            obj.model.lambda_c= 15;                             %poisson average rate of uniform clutter (per scan)
            obj.model.range_c= [ -pi/2 pi/2; 0 2000 ];          %uniform clutter on r/theta
            obj.model.pdf_c= 1/prod(obj.model.range_c(:,2)-obj.model.range_c(:,1)); %uniform clutter density

            %output variables
            
            %filter parameters
            obj.filter.H_bth= 5;                    %requested number of birth components/hypotheses
            obj.filter.H_sur= 3000;                  %requested number of surviving components/hypotheses
            obj.filter.H_upd= 3000;                  %requested number of updated components/hypotheses
            obj.filter.H_max= 3000;                  %cap on number of posterior components/hypotheses
            obj.filter.hyp_threshold= 1e-15;        %pruning threshold for components/hypotheses
            
            obj.filter.L_max= 100;                  %limit on number of Gaussians in each track - not implemented yet
            obj.filter.elim_threshold= 1e-5;        %pruning threshold for Gaussians in each track - not implemented yet
            obj.filter.merge_threshold= 4;          %merging threshold for Gaussians in each track - not implemented yet
            
            obj.filter.P_G= 0.9999999;                           %gate size in percentage
            obj.filter.gamma= chi2inv(obj.filter.P_G,obj.model.z_dim);   %inv chi^2 dn gamma value
            obj.filter.gate_flag= 1;                             %gating on or off 1/0
            
            obj.filter.run_flag= 'disp';            %'disp' or 'silence' for on the fly output

            %initial prior
            obj.glmb_update.tt= cell(0,1);      %track table for GLMB (cell array of structs for individual tracks)
            obj.glmb_update.w= 1;               %vector of GLMB component/hypothesis weights
            obj.glmb_update.I= {[]};            %cell of GLMB component/hypothesis labels (labels are indices/entries in track table)
            obj.glmb_update.n= 0;               %vector of GLMB component/hypothesis cardinalities
            obj.glmb_update.cdn= 1;             %cardinality distribution of GLMB (vector of cardinality distribution probabilities)

        end
        
        function obj = track(obj, mea_now, k)
            %METHOD1 此处显示有关此方法的摘要
            %   此处显示详细说明
            %prediction and update
            glmb_predict= predict(obj.glmb_update, obj.model, obj.filter, k);
            obj.glmb_update= update(glmb_predict, obj.model, obj.filter, mea_now', k);
            
            %pruning and truncation
            obj.glmb_update= prune(obj.glmb_update, obj.filter);
            obj.glmb_update= cap(obj.glmb_update, obj.filter);
...
if filter.gate_flag
    ...
    meas_gate = gate_meas_ekf(meas,filter.gamma,model,m_tracks,P_tracks);
end
%create updated tracks (single target Bayes update)
m= size(meas_gate,2);                                   %number of measurements
...
for emm= 1:m
    for tabidx= 1:length(glmb_predict.tt)
        ...
        [qz_temp,m_temp,P_temp] = ekf_update_multiple(meas(:,emm),model,glmb_predict.tt{tabidx}.m,glmb_predict.tt{tabidx}.P);      %ekf update for this track and this measurement
